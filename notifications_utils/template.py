...
class Template():
    ...
    def __init__(
        ...
        self.redact_missing = redact_missing
    ...
    def __str__(self):
        return Markup(
            Field(self.content, self.values, html='escape', redact_missing=self.redact_missing)
        )
...
class SMSPreviewTemplate(SMSMessageTemplate):
    ...
    def __str__(self):
        ...
        return Markup(self.jinja_template.render({
            ...
            'body': Take.as_field(
                self.content, self.values, html='escape', redact_missing=self.redact_missing
            ).then(
...
class WithSubjectTemplate(Template):
    ...
    def __init__(
        ...
        super().__init__(template, values, redact_missing=redact_missing)

    @property
    def subject(self):
        return Markup(Field(self._subject, self.values, html='escape', redact_missing=self.redact_missing))
...
class EmailPreviewTemplate(WithSubjectTemplate):
    ...
    def __str__(self):
        return Markup(self.jinja_template.render({
            'body': get_html_email_body(
                self.content, self.values, redact_missing=self.redact_missing
            ),
    ...
    @property
    def subject(self):
        return str(Field(self._subject, self.values, html='escape', redact_missing=self.redact_missing))
...
class LetterPreviewTemplate(WithSubjectTemplate):
    ...
    def __str__(self):
        return Markup(self.jinja_template.render({
            ...
            'message': Take.as_field(
                ...
                html='escape',
                markdown_lists=True,
                redact_missing=self.redact_missing,
            ).then(
                strip_pipes
            ).then(
                prepare_newlines_for_markdown
    ...
    def subject(self):
        return Take.as_field(
            self._subject, self.values, redact_missing=self.redact_missing, html='escape'
        ).then(
            strip_pipes
        ).then(
            strip_dvla_markup
...
def get_html_email_body(template_content, template_values, redact_missing=False):

    return Take.as_field(
        template_content, template_values, html='escape', markdown_lists=True, redact_missing=redact_missing
    ).then(
        unlink_govuk_escaped
    ).then(
        prepare_newlines_for_markdown
